...
*=============================================================================*
      ...
      SUBROUTINE saver1(it,itfix,iw,iwfix,nz,izout,sirrad,saflux,svi_zw,
...
*=============================================================================*
     ...
     $   rate,valj,svr_zs,svj_zj,svr_zt,svj_zt,svr_ts,svj_tj)
...
*=============================================================================*
      ...
      SUBROUTINE outpt1( outfil,iout,lirrad,laflux,lrates,ljvals,lmmech,
...
* spectral irradiance:
 ...
 1100   FORMAT('Spectral Irradiance,W m-2 nm-1')
         ...
         IF(itfix .GT. 0) THEN
            ...
            ENDDO
         ENDIF
 1110     FORMAT('values at t = ',F10.3,' hrs.',2x,'sza = ',F10.3)
 ...
 1160     FORMAT('Columns: wavelength (nm),times (hrs.) or sza (deg.)')
 ...
 1200    FORMAT('Columns: altitude (km),times (hrs.) or sza (deg.)')
...
* spectral actinic fluxes:
 ...
 1230     FORMAT('Spectral actinic flux,quanta cm-2 s-1 nm-1')
         ...
         IF(itfix .GT. 0) THEN
            ...
            ENDDO
         ENDIF
 1240     FORMAT('values at t = ',F10.3,' hrs.',2x,'sza = ',F10.3)
 ...
 1290     FORMAT('Columns: wavelength (nm),times (hrs.) or sza (deg.)')
 ...
 1330    FORMAT('Columns: altitude (km),times (hrs.) or sza (deg.)')
...
* dose rates
 ...
 1370     FORMAT(I4,' = ',A50)
         ...
         IF(itfix .GT. 0) THEN
            ...
            ENDDO
         ENDIF
 1380     FORMAT('values at t = ',F10.3,' hrs.',2x,'sza = ',F10.3)
 ...
 1410     FORMAT(0PF11.6,650(1PE11.3))
         ...
         IF(izout .GT. 0) THEN
            ...
            DO it = 1,nt
               WRITE(iout,1450) t(it),sza(it),(svr_ts(it,ims(i)),i = 1,
     $            nms)
            ENDDO
         ENDIF
...
* j-values
 ...
 1510     FORMAT(I4,' = ',A50)
         ...
         IF(itfix .GT. 0) THEN
            ...
            ENDDO
         ENDIF
 1520     FORMAT('values at t = ',F10.3,' hrs.',2x,'sza = ',F10.3)
 ...
 1550     FORMAT(0PF11.6,650(1PE11.3))
         ...
         IF(izout .GT. 0) THEN
            ...
            DO it = 1,nt
               WRITE(iout,1590) t(it),sza(it),(svj_tj(it,imj(i)),i = 1,
     $            nmj)
            ENDDO
         ENDIF
 ...
 1635    FORMAT(60('-'))
      ...
      IF(lmmech) THEN
         CALL outpt2(outfil,iout,izout,z,tlev,airden,nw,wl,wc,wu,svf_tw,
...
*=============================================================================*
      ...
      SUBROUTINE outpt2(outfil,iout,izout,z,tlev,airden,nw,wl,wc,wu,
     $   savsaf,nt,t,nj,jlabel,savjvl)
